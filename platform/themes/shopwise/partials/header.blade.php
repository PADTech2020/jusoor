...
<html lang="{{ app()->getLocale() }}">
    ...
    @php
        if (is_plugin_active('ecommerce')) {
            $categories = get_product_categories(['status' => \Botble\Base\Enums\BaseStatusEnum::PUBLISHED], ['slugable', 'children', 'children.slugable', 'metadata'], [], true);
        } else {
            $categories = [];
        }
    @endphp
    ...
    <header class="header_wrap @if (theme_option('enable_sticky_header', 'yes') == 'yes') fixed-top header_with_topbar @endif">
        ...
        <div class="bottom_header light_skin main_menu_uppercase bg_dark @if (url()->current() === route('public.single')) mb-4 @endif">
            <div class="container">
                <div class="row">
                    <div class="col-lg-3 col-md-4 col-sm-6 col-4">
                        ...
                        <div class="categories_wrap">
                            <button type="button" data-toggle="collapse" data-target="#navCatContent" aria-expanded="false" class="categories_btn">
                                ...
                                <div id="navCatContent" class="@if (url()->current() === route('public.single') && theme_option('collapsing_product_categories_on_homepage', 'no') == 'no') nav_cat @endif navbar collapse">
                                <ul>
                                    @foreach($categories as $category)
                                        @if ($loop->index < 10)
